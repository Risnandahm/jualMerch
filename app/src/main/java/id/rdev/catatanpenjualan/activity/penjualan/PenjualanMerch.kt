...
import kotlinx.android.synthetic.main.activity_penjualan_merch.*
import org.jetbrains.anko.*
import org.jetbrains.anko.sdk25.coroutines.onClick

class PenjualanMerch : BaseActivity(), PenjualanView {
    ...
    // merch yang dipilih saat pencarian
    private var merchNow : Merch? = null


    override fun onCreate(savedInstanceState: Bundle?) {
        cekSesi(this)
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_penjualan_merch)
    ...
    override fun onSuccessGetKeranjang(keranjang: List<Keranjang?>?) {
        ...
        rvKeranjangMerch.adapter = PenjualanAdapter(penjualan, object : PenjualanAdapter.OnDelete{
    ...
    private fun moveToKeranjang() {
        ...
        if (qtys.isNotBlank()) {
            val qty = qtys.toInt()
            penjualanPresenter.addItemToKeranjang(qty, merchNow, keranjangNow)
        } else {
    ...
    override fun onFailedGetKeranjang(msg: String?) {
        Log.d("PenjualanMerch", msg ?: "")
    }

    // search
    private fun setAction() {
        // saat mengetik di form search
        etSearchMerch.addTextChangedListener {
            val input = etSearchMerch.text.toString()
            ...
            if (input == "") {
                ...
                doAsync {
                    penjualanPresenter.searchMerch(input, user?.idUser)
                }
            }
        }
        ...
        btBayar.setOnClickListener {
            if (totalQty > 0) {
                alert {
                    ...
                    yesButton {
                        penjualanPresenter.jualMerch(user?.idUser, Integer.parseInt(keranjangNow?.idKeranjang), KeranjangStatus.TERJUAL.status,
                            totalQty, totalHarga)
                    }
                    noButton {  }
                }.show()
        ...
        imKeranjangDelete.setOnClickListener {
            if (listKeranjang?.size ?: 0 > 1) {
                alert {
                    ...
                    yesButton {
                        penjualanPresenter.jualMerch(user?.idUser, Integer.parseInt(keranjangNow?.idKeranjang), KeranjangStatus.TERHAPUS.status,
                            totalQty, totalHarga)
                    }
                    noButton {  }
                }.show()
    ...
    override fun onSuccessSearchItem(vs: List<Merch?>?) {
        // biar nama aja yang tampil
        val onlyName = merchs?.map {
            it?.namaMerch
        }
        ...
        lvResultSearch.setOnItemClickListener { _, _, position, _ ->
            merchNow = merchs?.get(position)
            etSearchMerch.setText(merchNow?.barcode)
            etQty.requestFocus()
        }
    }
    ...
    override fun onSuccessJualMerch(msg: String?) {
        keranjangIndex = 0
        getKeranjang()
    }
    override fun onFailedJualMerch(msg: String?) {
        Log.d("PenjualanMerch", msg ?: "")
    }
