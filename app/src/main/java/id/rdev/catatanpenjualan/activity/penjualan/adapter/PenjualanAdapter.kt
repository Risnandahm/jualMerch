...
class PenjualanAdapter(val listMerch: List<Penjualan?>?, val onDelete: OnDelete) : RecyclerView.Adapter<PenjualanAdapter.MyHolder>() {
    ...
    override fun getItemCount(): Int = listMerch?.size ?: 0

    override fun onBindViewHolder(holder: MyHolder, position: Int) {
        holder.bind(listMerch?.get(position), position)

        val penjualan = listMerch?.get(position)
    ...
    class MyHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(penjualan: Penjualan?, position : Int) {
            ...
            itemView.tvKeranjangName.text = penjualan?.namaMerch
